...
/// <summary>
/// 敌人（辅助回血类）2
/// Health: 27 | Atk: 0 | 攻击类型: 单体
/// 行为: 全体队友回复2点体力（不攻击）
/// </summary>
public class EnemyHealer2 : EnemyController
{
    [Header("Healer Enemy Settings")]
    private int healAmount = 2;
    
    protected new int initialHealth = 27;
    protected new int initialMaxHealth = 27;
    ...
    protected override IEnumerator GetActionCoroutine(ActionCommand command)
    {
        if (command.ActionType == ActionType.Attack)
        {
            ...
            // 只回血，不攻击
            Debug.Log($"[EnemyHealer2] {gameObject.name} 给所有队友恢复体力 (+{healAmount})");
    ...
    private IEnumerator HealAllies()
    {
        ...
        foreach (var ally in allies)
        {
            if (ally.IsAlive())
            {
                ...
                Debug.Log($"[EnemyHealer2] {ally.gameObject.name} 恢复了 {healAmount} 点体力");
            }
        }
