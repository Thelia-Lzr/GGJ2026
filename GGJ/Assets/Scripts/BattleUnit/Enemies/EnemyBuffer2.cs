...
/// <summary>
/// 敌人（辅助buff类）兼肉盾 2
/// Health: 22 | Atk: 5 | 攻击类型: 单体
/// 行为: 普攻*1 or 全体队友本回合攻击力+2
/// </summary>
public class EnemyBuffer2 : EnemyController
{
    ...
    protected override IEnumerator GetActionCoroutine(ActionCommand command)
    {
        if (command.ActionType == ActionType.Attack)
        {
            ...
            {
                // 给所有队友加攻击力BUFF
                Debug.Log($"[EnemyBuffer2] {gameObject.name} 给所有队友加攻击力BUFF (+{buffAmount})");
    ...
    private IEnumerator BuffAllies()
    {
        ...
        foreach (var ally in allies)
        {
            if (ally.IsAlive())
            {
                ...
                Debug.Log($"[EnemyBuffer2] {ally.gameObject.name} 获得攻击力+{buffAmount} BUFF");
            }
        }
