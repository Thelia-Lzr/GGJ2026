...
/// </summary>
public class EnemyDebuffer3 : EnemyController
{
    ...
    protected override IEnumerator GetActionCoroutine(ActionCommand command)
    {
        if (command.ActionType == ActionType.Attack)
        {
            ...
            if (command.Target != null && command.Target.IsAlive())
            {
                ...
                {
                    Debug.Log($"[EnemyDebuffer3] {gameObject.name} 给 {command.Target.gameObject.name} 降低攻击力 (-{debuffAmount})");
    ...
    private IEnumerator DebuffTarget(BattleUnit target)
    {
        if (target != null && target.IsAlive())
        {
            ...
            Debug.Log($"[EnemyDebuffer3] {target.gameObject.name} 攻击力降低 {debuffAmount}");
        }
