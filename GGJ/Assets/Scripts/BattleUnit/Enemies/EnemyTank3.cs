...
/// <summary>
/// 敌人（攻击单人类）兼肉盾 3
/// Health: 33 | Atk: 7 | 攻击类型: 单体
/// 行为: 普攻*1 or 蓄力1回合 普攻*2.5（向上取整）
/// </summary>
public class EnemyTank3 : EnemyController
{
    ...
    protected new int initialHealth = 33;
    protected new int initialMaxHealth = 33;
    ...
    protected override void OnStatusAppliedHandler(StatusEffect effect)
    {
        ...
        {
            Debug.Log($"[EnemyTank3] {gameObject.name} 被眩晕！蓄力计数器重置。");
            chargeCounter = 0;
        }
    }
    ...
    protected override void OnStatusRemovedHandler(StatusEffect effect)
    {
        ...
        if (effect is Stunned)
        {
            Debug.Log($"[EnemyTank3] {gameObject.name} 眩晕解除。");
        }
    }
    ...
    protected override IEnumerator GetActionCoroutine(ActionCommand command)
    {
        if (command.ActionType == ActionType.Attack)
        {
            ...
            {
                Debug.Log($"[EnemyTank3] {gameObject.name} 释放蓄力攻击！");
            ...
            {
                chargeCounter++;
                Debug.Log($"[EnemyTank3] {gameObject.name} 继续蓄力 ({chargeCounter}/{chargeTime})");
            ...
            else
            {
                if (Random.value > 0.5f)
                {
                    // 开始蓄力
                    chargeCounter++;
                    Debug.Log($"[EnemyTank3] {gameObject.name} 开始蓄力 ({chargeCounter}/{chargeTime})");
                    yield return new WaitForSeconds(0.5f);
                }
                else
                {
    ...
    private IEnumerator AttackSingleRoundUp(BattleUnit target, float multiplier)
    {
        if (target != null && target.IsAlive())
        {
            ...
            Debug.Log($"[EnemyTank3] {gameObject.name} 对 {target.gameObject.name} 造成 {damage} 点蓄力伤害");
        }
